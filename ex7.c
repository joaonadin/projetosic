...
//inicializando parametros do obstaculo
nxb=(nx)/5;  //obstaculo começa em x=100
nxe=nxb+50;	 //obstaculo termina em x=150
nyb=30;      //obstaculo começa em y=30
nye=nyb+20;  //obstaculo termina em y=50
...
//loop do obstaculo
	//a) bounce-back topo e base
	for(int x=nxb;x<nxe;x++){
		f[4][x][nyb]=f[2][x][nyb];
		f[7][x][nyb]=f[5][x][nyb];
		f[8][x][nyb]=f[6][x][nyb];
		f[2][x][nye]=f[4][x][nye];
	...
	//b) bounce-back entrada e saida
	for(int y=nyb;y<nye;y++){
		f[3][nxb-1][y]=f[1][nxb-1][y];
		f[7][nxb-1][y]=f[5][nxb-1][y];
		f[6][nxb-1][y]=f[8][nxb-1][y];
		f[1][nxe-1][y]=f[3][nxe-1][y];
		f[5][nxe-1][y]=f[7][nxe-1][y];
		f[8][nxe-1][y]=f[6][nxe-1][y];
		}
	//c) sem velocidade dentro do bloco
	for(int x=nxb;x<nxe;x++){
		for(int y=nyb;y<nye;y++){
			u[x][y]=0;
			v[x][y]=0;
...
// Escreve a matriz 'u' no arquivo
	FILE *fileu = fopen("/home/joao/Documents/PraticaLBM/u3.txt", "w");
    for (int i = 0; i < nx; i++) {
        for (int j = 0; j < ny; j++) {
    ...
    FILE *filev = fopen("/home/joao/Documents/PraticaLBM/v3.txt", "w");
    for (int i = 0; i < nx; i++) {
        for (int j = 0; j < ny; j++) {
