...
while(erro>tol){
	...
	//e) regiao solida
	for(int y=1;y<10;y++){             //zerando velocidades
		for(int x=0;x<nx;x++){
			u[x][y]=0;
			v[x][y]=0;
			u[x][ny-1-y]=0;
	...
	for(int x=0;x<nx;x++){             //bounce-back
		f[5][x][10]=f[7][x][10];
		f[2][x][10]=f[4][x][10];
		f[6][x][10]=f[8][x][10];
		f[7][x][ny-10]=f[5][x][ny-10];
		f[4][x][ny-10]=f[2][x][ny-10];
		f[8][x][ny-10]=f[6][x][ny-10];
	}
	...
	/*Colisao termica*/
		for (int x=0;x<nx;x++){
			for (int y=0;y<ny;y++){
				t1=pow(u[x][y],2)+pow(v[x][y],2);
				mg=omegag;
				if(y<10 || y>ny-10){    //condutividade maior para a parede
					mg=mg*0.1;}
				for(int i=0;i<9;i++){
					t2=u[x][y]*cx[i]+v[x][y]*cy[i];
					geq[i][x][y]=w[i]*temperatura[x][y]*(1+3*t2+4.5*pow(t2,2)-1.5*t1);
					g[i][x][y]=g[i][x][y]*(1-mg)+mg*geq[i][x][y];
				}
			}
		}
...
// Escreve a matriz 'u' no arquivo
	FILE *fileu = fopen("/home/joao/Documents/PraticaLBM/utermico3.txt", "w");
    for (int i = 0; i < nx; i++) {
        for (int j = 0; j < ny; j++) {
    ...
    FILE *filev = fopen("/home/joao/Documents/PraticaLBM/vtermico3.txt", "w");
    for (int i = 0; i < nx; i++) {
        for (int j = 0; j < ny; j++) {
	...
	FILE *filet = fopen("/home/joao/Documents/PraticaLBM/temperaturasescoamento3.txt", "w");
    for (int i = 0; i < nx; i++) {
        for (int j = 0; j < ny; j++) {
